...
Delta=-400. #superconducting gap
w=800. #hopping amplitude
V0=1400. #peak of the on-site potential
dedge=3. # anyons' positions away from the edge
mu0=0. #baseline potential
...
s10=tf.constant([0.],dtype=tf.float64)
s20=tf.constant([0.],dtype=tf.float64)
PotentialProfile0=potential_profile_I(N, dedge, mu0, V0, s10, s20)
KitaevH0=tf.tensor_scatter_nd_update( KitaevH, [[i,i] for i in range(6*N+2)], PotentialProfile0[0] )
eig0=tf.linalg.eigh(KitaevH0)

print('Zero mode energies at t=0:')
print( tf.math.real(eig0[0])[3*N:3*N+2].numpy() )
print('Gap at t=0:')
print( tf.math.real(eig0[0])[3*N+2].numpy()-tf.math.real(eig0[0])[3*N+1].numpy() )
...
s1T=tf.constant([1.],dtype=tf.float64)
s2T=tf.constant([1.],dtype=tf.float64)
PotentialProfileT=potential_profile_IV(N, dedge, mu0, V0, s1T, s2T)
KitaevHT=tf.tensor_scatter_nd_update( KitaevH, [[i,i] for i in range(6*N+2)], PotentialProfileT[0] )
eigT=tf.linalg.eigh(KitaevHT)
print('Zero mode energies at t=T:')
print( tf.math.real(eigT[0])[3*N:3*N+2].numpy() )
print('Gap at t=T:')
print( tf.math.real(eigT[0])[3*N+2].numpy()-tf.math.real(eigT[0])[3*N+1].numpy() )
...
@tf.function
def loss_fn(s12pred):
	s1VarI,s2VarI,s1VarII,s2VarII,s1VarIII,s2VarIII,s1VarIV,s2VarIV = profiles_from_NN(s12pred)
	
	zero=tf.constant([0.],dtype=tf.float64)
	one=tf.constant([1.],dtype=tf.float64)
	UI=transport_operator_map(N,KitaevH,mu0,V0,dedge,tf.concat([zero,s1VarI,one],-1),tf.concat([zero,s2VarI],-1),dt,'I')
	UII=transport_operator_map(N,KitaevH,mu0,V0,dedge,s1VarII,tf.concat([s2VarII,one],-1),dt,'II')
	UIII=transport_operator_map(N,KitaevH,mu0,V0,dedge,tf.concat([s1VarIII,one],-1),s2VarIII,dt,'III')
	UIV=transport_operator_map(N,KitaevH,mu0,V0,dedge,tf.concat([s1VarIV,one],-1),tf.concat([s2VarIV,one],-1),dt,'IV')
...
tVec=tf.reshape( tf.cast( tf.linspace(0.,1.,NT-1), dtype=tf.float64 ), [-1,1] )
...
s1VarI,s2VarI,s1VarII,s2VarII,s1VarIII,s2VarIII,s1VarIV,s2VarIV = profiles_from_NN(s12pred)
plot_profiles(NT,s1VarI,s2VarI,s1VarII,s2VarII,s1VarIII,s2VarIII,s1VarIV,s2VarIV)
plt.show()
