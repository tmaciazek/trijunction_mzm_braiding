...
def transport_operator_map(N, Ham0, mu0, V0, dedge, s1Vec, s2Vec, dt, stage):
    ...
    Idt = -dt * tf.cast(tf.complex(0.0, 1.0), dtype=tf.complex128)
...
@tf.function
def transport_operator_loop(
    N, Ham0, mu0, V0, dedge, s1Vec, s2Vec, dt, stage, ParallelIterations
):
    """
    generates the adiabatic evolution operator
    ...
    U = tf.linalg.expm(
        tf.math.scalar_mul(
            Idt,
            tf.tensor_scatter_nd_update(
                Ham0, [[i, i] for i in range(6 * N + 2)], PotentialProfiles[0]
            ),
        )
    )
    dtEvolution = lambda istep, U: (
        tf.add(istep, 1),
        tf.linalg.matmul(
            tf.linalg.expm(
                tf.math.scalar_mul(
                    ...
                    tf.tensor_scatter_nd_update(
                        Ham0,
                        [[i, i] for i in range(6 * N + 2)],
                        PotentialProfiles[istep],
                    ),
                )
            ),
            U,
    ...
    return tf.while_loop(
        c, dtEvolution, [istep, U], parallel_iterations=ParallelIterations
    )[1]
...
def linear_motion_profiles(NT):
    s1I = tf.Variable(tf.cast(tf.linspace(0.0, 1.0, NT), dtype=tf.float64)[1:-1])
    s2I = tf.Variable(tf.zeros(NT, dtype=tf.float64)[1:])

    s1II = tf.Variable(tf.cast(tf.linspace(0.0, 1.0, NT), dtype=tf.float64)[1:])
    s2II = tf.Variable(tf.cast(tf.linspace(0.0, 1.0, NT), dtype=tf.float64)[1:-1])

    s1III = tf.Variable(tf.cast(tf.linspace(0.0, 1.0, NT), dtype=tf.float64)[1:-1])
    s2III = tf.Variable(tf.cast(tf.linspace(0.0, 1.0, NT), dtype=tf.float64)[1:])

    s1IV = tf.Variable(tf.cast(tf.linspace(0.0, 1.0, NT), dtype=tf.float64)[1:-1])
...
def sinusoidal_motion_profiles(NT):
    rampup = 2.0 * tf.math.square(
        tf.math.sin(tfpi * tf.cast(tf.linspace(0.0, 1.0, NT), dtype=tf.float64) / 4.0)
    )
    rampdown = tf.math.sin(
        tfpi * tf.cast(tf.linspace(0.0, 1.0, NT), dtype=tf.float64) / 2.0
    )
